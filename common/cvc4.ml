...
let log_cvc4_formula = ref false
let _valid = true
let _invalid = false
let _sat = true
let _unsat = false
let cvc4_log = ref stdout
...
(* let set_log_file fn = *)
(*   log_cvc4_formula := true; *)
(*   if fn = "" then *)
(* 	cvc4_log := open_out "formula.cvc" *)
(*   else (\*if Sys.file_exists fn then *)
(* 	failwith "--log-cvc4: file exists" *)
(*   else*\) *)
(* 	begin *)
(* 		cvc4_log := open_out fn (\* opens fn for writing and returns an output channel for fn - cvc4_log is the output channel*\); *)
(* 		(\* output_string !cvc4_log cvc4_command *\) *)
(* 	end *)

let set_log_file () :  unit=
  log_cvc4_formula := true;
  cvc4_log := open_log_out "allinput.cvc4"

let run_cvc4 (input : string) : unit =
  begin
    let chn = open_out infilename in
    output_string chn input;
    close_out chn;
    ignore (Sys.command cvc4_command)
  end

let log_answer_cvc4 (answer : string) : unit =
  if !log_cvc4_formula then
    begin
      output_string !cvc4_log answer;
      flush !cvc4_log
    end

let return_answer (inchn: in_channel) (answer_to_log: string) (answer:bool option): bool option =
  let () = close_in inchn in
  let () = log_answer_cvc4 ("%%%Res: " ^ answer_to_log ^ "\n\n") in
  answer

let rec cvc4_of_spec_var (sv : CP.spec_var) = match sv with
  | CP.SpecVar (_, v, p) -> v ^ (if CP.is_primed sv then "PRMD" else "")

and cvc4_of_exp a = match a with
  | CP.Null _ -> "0"
  | CP.Var (sv, _) -> cvc4_of_spec_var sv
  | CP.IConst (i, _) -> string_of_int i
  | CP.FConst _ -> failwith ("[cvc4ite.ml]: ERROR in constraints (float should not appear here)")
  | CP.Add (a1, a2, _) ->  (cvc4_of_exp a1) ^ " + " ^ (cvc4_of_exp a2)
  | CP.Subtract (a1, a2, _) ->  (cvc4_of_exp a1) ^ " - " ^ (cvc4_of_exp a2)
  | CP.Mult (a1, a2, _) -> (cvc4_of_exp a1) ^ " * " ^ (cvc4_of_exp a2)
  | CP.Div (a1, a2, _) -> failwith ("[cvc4.ml]: divide is not supported.")
  | CP.Max _ 
  | CP.Min _ -> failwith ("cvc4_of_exp: min/max should not appear here")
  | CP.TypeCast _ -> failwith "cvc4_of_exp : TypeCast cannot be handled"
  | CP.Bag ([], _) -> ""
  | CP.Bag _ | CP.BagUnion _ | CP.BagIntersect _ | CP.BagDiff _ ->
    failwith ("[cvc4.ml]: ERROR in constraints (set should not appear here)");
  | CP.List _ | CP.ListCons _ | CP.ListHead _ | CP.ListTail _ | CP.ListLength _ | CP.ListAppend _ | CP.ListReverse _ ->
    failwith ("Lists are not supported in cvc4")
  | CP.Func _ -> failwith ("Functions are not supported in cvc4")
  | CP.ArrayAt _ -> (* An Hoa *)
    failwith ("Arrays are not supported in cvc4")
  | CP.AConst _ -> failwith ("aconst not supported in cvc4")
  | CP.Level _ -> failwith ("level should not be here in cvc4")
  | CP.Tsconst _ -> failwith ("tsconst not supported in cvc4")
  | CP.Bptriple _ -> failwith ("cvc4.cvc4_of_exp: Bptriple should not appear here")
  | CP.Tup2 _ -> failwith ("cvc4.cvc4_of_exp: Tup2 should not appear here")
  | CP.InfConst _ 
  |CP.NegInfConst _ -> failwith ("[cvc4ite.ml]: ERROR in constraints (\inf should not appear here)")
  | CP.Template t -> cvc4_of_exp (CP.exp_of_template t)

and cvc4_of_b_formula b =
  let (pf,_) = b in
  match pf with
  | CP.Frm (sv, _) -> (cvc4_of_spec_var sv) ^ " > 0"
  (* | CP.BConst (c, _) -> if c then "(TRUE)" else "(FALSE)" *)
  | CP.BConst (c, _) -> if c then "(0 = 0)" else "( 0 > 0)"
  (* | CP.BVar (sv, _) -> cvc4_of_spec_var sv *)
  | CP.BVar (sv, _) -> (cvc4_of_spec_var sv) ^ " > 0"
  | CP.Lt (a1, a2, _) -> (cvc4_of_exp a1) ^ " < " ^ (cvc4_of_exp a2)
  | CP.Lte (a1, a2, _) -> (cvc4_of_exp a1) ^ " <= " ^ (cvc4_of_exp a2)
  | CP.Gt (a1, a2, _) -> (cvc4_of_exp a1) ^ " > " ^ (cvc4_of_exp a2)
  | CP.Gte (a1, a2, _) -> (cvc4_of_exp a1) ^ " >= " ^ (cvc4_of_exp a2)
  | CP.Eq (a1, a2, _) -> 
  ...
  | CP.Neq (a1, a2, _) -> 
    if CP.is_null a2 then 
      ...
      (cvc4_of_exp a1) ^ " /= " ^ (cvc4_of_exp a2)
  | CP.EqMax (a1, a2, a3, _) ->
    let a1str = cvc4_of_exp a1 in
    let a2str = cvc4_of_exp a2 in
    let a3str = cvc4_of_exp a3 in
  ...
  | CP.BagIn (v, e, l)			-> " in(" ^ (cvc4_of_spec_var v) ^ ", " ^ (cvc4_of_exp e) ^ ")"
  | CP.BagNotIn (v, e, l)	-> " NOT(in(" ^ (cvc4_of_spec_var v) ^ ", " ^ (cvc4_of_exp e) ^"))"
  | CP.BagSub (e1, e2, l)	-> " subset(" ^ cvc4_of_exp e1 ^ ", " ^ cvc4_of_exp e2 ^ ")"
  | CP.BagMax _ | CP.BagMin _ -> failwith ("cvc4_of_b_formula: BagMax/BagMin should not appear here.\n")
  (* | CP.VarPerm _ -> failwith ("VarPerm are not supported in cvc4") *)
  | CP.ListIn _
  | CP.ListNotIn _
  | CP.ListAllN _
  | CP.ListPerm _ -> failwith ("Lists are not supported in cvc4")
  | CP.RelForm _ -> failwith ("Relations are not supported in cvc4") (* An Hoa *)
  | CP.SubAnn _ -> failwith ("SubAnn not supported in cvc4")
...
and cvc4_of_formula f = match f with
  | CP.BForm (b,_) -> 
    begin
      match (fst CP.drop_complex_ops) (fst b) with
      | None -> "(" ^ (cvc4_of_b_formula b) ^ ")"
      | Some f -> cvc4_of_formula f
    end
  | CP.And (p1, p2, _) -> "(" ^ (cvc4_of_formula p1) ^ " AND " ^ (cvc4_of_formula p2) ^ ")"
  | CP.AndList _ -> Gen.report_error no_pos "cvc4.ml: encountered AndList, should have been already handled"
  | CP.Or (p1, p2,_, _) -> "(" ^ (cvc4_of_formula p1) ^ " OR " ^ (cvc4_of_formula p2) ^ ")"
  | CP.Not (p,_, _) ->
    begin
      match p with
      | CP.BForm ((CP.BVar (bv, _), _), _) -> (cvc4_of_spec_var bv) ^ " <= 0"
      | _ -> "(NOT (" ^ (cvc4_of_formula p) ^ "))"
    end
  | CP.Forall (sv, p,_, _) ->
    let typ_str = cvc4_of_sv_type sv in
    "(FORALL (" ^ (cvc4_of_spec_var sv) ^ ": " ^ typ_str ^ "): " ^ (cvc4_of_formula p) ^ ")"
  | CP.Exists (sv, p, _,_) -> 
    let typ_str = cvc4_of_sv_type sv in
    "(EXISTS (" ^ (cvc4_of_spec_var sv) ^ ": " ^ typ_str ^ "): " ^ (cvc4_of_formula p) ^ ")"
...
let log_text_to_cvc4 (str : string) : unit =
  if !log_cvc4_formula then
    begin
      output_string !cvc4_log str
    end
...
let rec get_answer (process: GlobProver.prover_process_t) : string =
  ...
  with
    _ ->   (print_string ("\nexception getting the answer from cvc4: \n" ); flush stdout); ""

(*reads the prompt that cvc4 outputs when running in incremental mode *)
let rec read_prompt (process: GlobProver.prover_process_t) : string =
  ...
  with
  |  _ ->   (print_string ("\nexception while reading cvc4 promp \n" ); flush stdout); ""

(*send one command to cvc4 process without expecting any answer*)
let send_cmd (process: GlobProver.prover_process_t) (cmd: string): unit = 
  try
    ...
    let () = log_text_to_cvc4  cmd in    
    let () = flush process.outchannel in
    ()
  with
    _ ->  (print_string ("\nerror when sending a command: \n" ); flush stdout); ()

(*send one command to cvc4 process and waits for its answer*)
...
(*saves the current state in the active context of cvc4process*)
let cvc4_push (process: prover_process_t) = 
  let cmd = "PUSH;\n" in
  send_cmd process cmd

(*returns to the state before the last call of PUSH*)
let cvc4_pop (process: prover_process_t) = 
  let cmd = "POP;\n" in
  send_cmd process cmd
...
let start () : prover_process_t =
  ...
  let () = Procutils.PrvComms.start !log_cvc4_formula !cvc4_log ("cvc4", "cvc4", [|"cvc4"; "--interactive";(* "+printResults"*)|]) set_process (fun ()->()) in
...
(*all the formulas that shall be send to cvc4 process have to be transformed in cvc4 input language *)
let prepare_formula_for_sending (f : CP.formula) : string =
  cvc4_of_formula f 

(*declares the set of variables included in vars (list of variables) *)
let cvc4_declare_list_of_vars (process: prover_process_t) (vars: CP.spec_var list) =
  let int_vars, bool_vars, bag_vars = split_vars vars in
  let bag_var_decls = 
    if Gen.is_empty bag_vars then "" 
    else (String.concat ", " (List.map cvc4_of_spec_var bag_vars)) ^ ": SET;" in
  let int_var_decls = 
    if Gen.is_empty int_vars then "" 
    else (String.concat ", " (List.map cvc4_of_spec_var int_vars)) ^ ": INT;" in
  let bool_var_decls =
    if Gen.is_empty bool_vars then ""
    else (String.concat ", " (List.map cvc4_of_spec_var bool_vars)) ^ ": INT;" in (* BOOLEAN *)
...
let cvc4_declare_vars_of_formula (process: prover_process_t) (f: CP.formula) = 
  let all_fv = CP.remove_dups_svl (CP.fv f) in
  cvc4_declare_list_of_vars process all_fv

(*asserts a condition to the context active in prover_process_t*)
let cvc4_assert (process: prover_process_t) (f : CP.formula) =
  (* let () = print_string ("\nCvc4.ml: cvc4_assert ") in *)
  let f_str = prepare_formula_for_sending f in
  let cmd = "ASSERT " ^ f_str ^ ";\n" in
  send_cmd process cmd

(*checks for implication*)
let cvc4_query (process: prover_process_t) (f : CP.formula) : bool option * string = 
  (* let () = print_string ("\nCvc4.ml: cvc4_query ") in *)
...
let cvc4_checksat (process: prover_process_t) (f : CP.formula): bool option * string= 
...
(* let cvc4_restart_query (process: prover_process_t) (f: CP.formula) : bool option = 
  ...
  let () = log_text_to_cvc4 restart_str in
...
(*sends the query command to "process" and returns the answer given by cvc4*)
let imply_helper (process: prover_process_t) (send_ante: bool) (ante : CP.formula) (conseq : CP.formula) (imp_no : string) : bool option =
  ...
  let () = 
    if (send_ante) then
      ...
      let () = cvc4_declare_list_of_vars process (ante_fv @ conseq_fv) in
      cvc4_assert process ante
    else cvc4_declare_vars_of_formula process (conseq) in
  let (answer, answer_str) = cvc4_query process conseq in
  let () = (log_text_to_cvc4  ("%%% Res: " ^ answer_str ^ " \n\n");  flush !cvc4_log) in
  answer

(*creates a new cvc4 process, sends the query command to the freshly created"process", stops the process and returns the answer given by cvc4*)
...
let imply_increm (process: (prover_process_t option * bool) option) (ante : CP.formula) (conseq : CP.formula) (imp_no : string) : bool =
  ...
  let result = match result0 with
    | Some f -> f
    | None -> begin
        (log_text_to_cvc4   "%%% imply --> invalid (from unknown)\n\n";  flush !cvc4_log); 
        _invalid  (* unknown is assumed to be false *)
        (*failwith "CVC4 is unable to perform implication check"*)
      end
  in
  result
...
let is_sat_increm (process: prover_process_t option) (f : CP.formula) (sat_no : string) : bool =
  ...
  let result = match result0 with
    | Some f -> f
    | None -> begin
        (log_text_to_cvc4   "%%% is_sat --> true (from unknown)\n\n";  flush !cvc4_log);
        (*failwith "CVC4 is unable to perform satisfiability check"*)
        _sat
      end
  in
  result
