...
class timelog =
  object (self)
    ...
    method dump = 
      ...
      if (not !Globals.print_min) then Debug.info_hprint (add_str "log(small)" (pr_pair string_of_float string_of_int )) (s,List.length small) no_pos;
      let tl_str = string_of_float (!time_limit_large) in
      if not(big==[]) then Debug.info_hprint (add_str ("log(big)(>"^tl_str^"s)("^s_big^")") (pr_pair string_of_float prL)) (b,big) no_pos;
