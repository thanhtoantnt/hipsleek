...
let rec_unfold_formula_of_hrel prog hrel_root hrel_args = 
  ...
  | Named d_name ->
    ...
    List.fold_left (fun f ptr ->
      ...
      let (hr, _) = x_add (C.add_raw_hp_rel ~caller:__LOC__) prog true true hr_args pos in
  ...
  let null_detect_trans eq_nulls hf=
    ...
    | ViewNode vn ->
      if String.compare (CP.name_of_spec_var vn.h_formula_view_node) null_name = 0 then
        let vdcecl = x_add C.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls vn.h_formula_view_name in
...
let check_tail_rec_rec_lemma_x prog lhs rhs l_reach_dns l_reach_vns r_reach_dns r_reach_vns =
  ...
  if r_reach_dns != [] then
    ...
    | [rvn] ->
      let rvdcl = x_add Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls rvn.h_formula_view_name in
...
let is_out_of_scope prog lvnode rvnode=
  if String.compare lvnode.h_formula_view_name rvnode.h_formula_view_name = 0 then
    let vdcl = Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls lvnode.h_formula_view_name in
    List.exists (fun (f, _) ->
...
let is_seg_view_br_fold_form_x prog ldnode lhs0 rvnode rhs0 remap conseq_pure_opt=
  ...
  let exist_full_fold lhs rvnode=
    let vdecl = Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls rvnode.h_formula_view_name in
  ...
  let seg_fold_view ldnode rvnode=
    let vdcl = Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls rvnode.h_formula_view_name in
...
let seg_fold_view2_x prog lvnode rvnode conseq rhs_b=
  let vdecl = Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls lvnode.h_formula_view_name in
...
let seg_fold_view_br_x prog ldnode rvnode ante conseq rhs_b=
  ...
  let rec find_first_seg_match ivars init_mtl fwd_seg_args fs res dist_segs0=
    ...
    | [] -> [],res,dist_segs0
  in
  let vdecl = x_add Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls rvnode.h_formula_view_name in
  (* get pos of fwd seg ptrs. todo: backward seg ptrs *)
...
let is_view_node_segmented vn prog =
  let vdcl = Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls vn.h_formula_view_name in
  ...
  let rec dfs_find_closure working_vns done_vns=
    match working_vns with
      ...
      | vn::rest -> if List.exists (fun vn1 -> string_eq vn vn1) done_vns then
          ...
          let vdclr = Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls vn in
