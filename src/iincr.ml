...
let extend_views iprog prog rev_formula_fnc trans_view_fnc ext_pred_names proc=
  ...
  let vdcls = List.map (x_add Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls) ( rev_cl_vns1) in
  let pure_extn_views = List.map (Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls) ext_pred_names in
  ...
  let new_vdclrs = List.map (Cast.look_up_view_def_raw __LOC__ prog.Cast.prog_view_decls) new_vns in
