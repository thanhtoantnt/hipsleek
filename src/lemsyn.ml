...
let gen_lemma_infer_x (prog) ass_stk hpdef_stk
  ...
  in
  (*END*)
  (* let vdef = x_add C.look_up_view_def_raw __LOC__ prog.C.prog_view_decls vnode.Cformula.h_formula_view_name in *)
  ...
  let rhs,n_hp = x_add (C.add_raw_hp_rel  ~caller:__LOC__) prog false false [(vself,I);(dnode1.Cformula.h_formula_data_node,NI)] no_pos in
  (*add ihpdecl*)
  let iprog = Iast.get_iprog () in
