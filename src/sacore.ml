...
let do_entail_check_x vars iprog cprog cs=
  ...
  let get_view_def vname=
    ...
    let vdef = (x_add Cast.look_up_view_def_raw __LOC__ cprog.Cast.prog_view_decls vname) in
  ...
  let find_greates_neg rcomplete (r_fs, r_unk_hpargs) f=
    ...
    if CP.mem_svl r svl then
      ...
      if not rcomplete then
        let (hf, n_hp) = x_add (Sautil.add_raw_hp_rel ~caller:__LOC__) prog is_pre true [(r, I)] pos in
...
let pred_split_ext iprog cprog proc_name ass_stk hpdef_stk
  ...
  let size_ext_hpdef hpdef=
    ...
    | ext_v::_ ->
      ...
      let n_lhs, n_hp = x_add (Sautil.add_raw_hp_rel ~caller:__LOC__) cprog true false (List.map (fun sv ->
...
let pred_norm_seg_x iprog prog unk_hps hp_defs=
  ...
  let generate_seg_pred orig_hp orig_all_args orig_root orig_args rec_fs dep_hp=
    ...
    let seg_def_lhs0, n_hp = x_add (Sautil.add_raw_hp_rel ~caller:__LOC__) prog true false (orig_hpcl.Cast.hp_vars_inst@[(seg_sv,NI)]) no_pos in
