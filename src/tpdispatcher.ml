...
let rec check_prover_existence prover_cmd_str =
  ...
  | prover::rest -> 
    ...
    let prover = 
      ...
      else if String.compare prover "mona" = 0 then try Gen.SysUti.which "mona" with Not_found -> ""
      else prover
    in
...
let set_tp user_flag tp_str =
  ...
  if tp_str = "parahip" then allow_norm := false else allow_norm:=true;
  (**********************************************)
